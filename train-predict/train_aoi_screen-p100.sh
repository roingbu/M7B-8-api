...
cleanup() {
    ...
    pkill -9 -f "train_mura_segment-p100.py" || true   # <--- 必须加上这一行！
...
# DOT：训练总轮数
export DOT_EPOCHS="${DOT_EPOCHS:-2000}" # 训练轮次
...
export DOT_NAME="${DOT_NAME:-mc9_4gpu_oversample_try0}" # 输出实验名
...
# MURA：训练总轮数
export SEG_EPOCHS="${SEG_EPOCHS:-2000}" # 训练轮次
# MURA：全局 batch（分割+mask 更占显存，通常比 DOT 更小）
export SEG_BATCH="${SEG_BATCH:-24}" # 全局 batch size，P100 默认 2
# MURA：名义 batch，作用同 DOT_NBS（与 SEG_BATCH 搭配控制梯度累加）
export SEG_NBS="${SEG_NBS:-96}" # 用于自动计算梯度累加
# MURA：本次实验在 runs/mura_segment 下的子目录名
export SEG_NAME="${SEG_NAME:-mc8_4gpu_oversample_try0}" # 输出实验名
# MURA：是否从上一次训练的 last.pt 继续
export SEG_RESUME="${SEG_RESUME:-0}" # 1=从 last.pt 继续训练

# P100 上默认使用 GPU 0，若是多卡 DDP 可设置 P100_DEVICE=0,1,2,3
