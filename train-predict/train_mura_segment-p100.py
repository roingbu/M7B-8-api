"""
MURA 缺陷分割训练 (Ultralytics YOLO，多通道 .npy) - P100 兼容版本

数据为 (H,W,8) uint8；dataset.yaml 需含 channels: 8。首层卷积由 data['channels'] 构建，
...
用法:
    cd /roing-workspace/train-predict && python train_mura_segment-p100.py
    python train_mura_segment-p100.py --model yolo26m-seg.pt --epochs 100
"""
...
PROJECT_DIR = "/roing-workspace/train-predict/runs/mura_segment_p100"


def parse_args():
    p = argparse.ArgumentParser(description="MURA 多通道分割训练 (P100 兼容)")
    p.add_argument(
        "--model",
        type=str,
...
def main():
    ...
    if not Path(args.model).is_file():
        raise FileNotFoundError(
            f"权重不存在: {args.model}\n请放置 yolo26m-seg.pt 到本地或改用 --model yolo26m-seg.pt 让 Ultralytics 自动下载"
        )
    ...
    metrics = model.val()
    print(f"\n{'=' * 50}")
    print(f"mIoU:     {metrics.seg.mean_iou:.4f}")
