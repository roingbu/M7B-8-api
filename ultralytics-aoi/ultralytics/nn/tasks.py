...
class BaseModel(torch.nn.Module):
    ...
    def load(self, weights, verbose=True):
        ...
        # Multi-channel: mean pretrained weights over input channels, then broadcast to all new in_channels.
        state_dict = self.state_dict()
        if first_conv not in updated_csd and first_conv in state_dict and first_conv in csd:
            w_new, w_old = state_dict[first_conv], csd[first_conv]
            out_n, in_n, kh, kw = w_new.shape
            out_o, in_o, kh_o, kw_o = w_old.shape
            if kh == kh_o and kw == kw_o and out_n == out_o and in_n != in_o:
                mean_w = w_old.mean(dim=1, keepdim=True)  # (out_o, 1, kh, kw)
                w_new.copy_(mean_w.expand(-1, in_n, -1, -1))
                len_updated_csd += 1
            elif kh == kh_o and kw == kw_o:
                c1, c2 = min(out_n, out_o), min(in_n, in_o)
                w_new[:c1, :c2].copy_(w_old[:c1, :c2])
                len_updated_csd += 1
        if verbose:
