...
class BaseTrainer:
    ...
    def _setup_train(self):
        ...
        if self.world_size > 1:
            # 多卡时可选：环境变量 YOLO_SYNC_BN=1 将 BN 换为 SyncBatchNorm，使各卡统计量同步
            if os.environ.get("YOLO_SYNC_BN", "0") == "1":
                LOGGER.info(f"{colorstr('SyncBatchNorm:')} converting BatchNorm for DDP (YOLO_SYNC_BN=1)")
